...
static wait_queue_head_t* tmp_read_wait;
static unsigned char is_reset = 0;
				...
				const struct usb_device_id *id)
{
  ...
  #if CONFIG_BLUEDROID //for 4.2
  //if bt is on
  if (is_reset==1 && tmp_read_wait) {
    hci_fake_hardware_error();
  }
  is_reset = 0;
  #endif
...
#if CONFIG_BLUEDROID //for 4.2
  if (test_bit(HCI_RUNNING, &hdev->flags)) { // bt is on
    tmp_read_wait = &data->read_wait;
    is_reset = 1;
  }
#endif
...
static void hci_fake_hardware_error()
{
	struct sk_buff *rtk_skb_copy = NULL;
	int len=3;
	uint8_t		fake_pkt[3]={HCI_EVENT_PKT,0x10,0x00};

	RTKBT_DBG("hci_fake_hardware_error");

	rtk_skb_copy= alloc_skb(len, GFP_ATOMIC);
	if (!rtk_skb_copy)
	{			
		RTKBT_DBG("allocat failed \n");
		return;
	}
	memcpy(skb_put(rtk_skb_copy, len), fake_pkt, len);

	enqueue(rtk_skb_copy);
	kfree_skb(rtk_skb_copy);
	
	wake_up_interruptible(tmp_read_wait);
}
/*
