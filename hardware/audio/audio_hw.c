...
static int throw_first_play_frame = 0;//add by xzd
enum tty_modes {
    TTY_MODE_OFF,
    TTY_MODE_VCO,
    TTY_MODE_HCO,
...
static int start_output_stream(struct sunxi_stream_out *out)
{
	...
	if (g_no_output && adev->mode == AUDIO_MODE_IN_CALL)
	{
		//ALOGW("mode in call, do not start stream");
		return 0;
	}
	if (adev->mode == AUDIO_MODE_MODE_FACTORY_TEST || adev->mode == AUDIO_MODE_FM)
	{
		//ALOGW("mode in call, do not start stream");
		return 0;
	}
                         ...
                         size_t bytes)
{
	...
	if(adev->out_device == AUDIO_DEVICE_OUT_SPEAKER) {
		if(throw_first_play_frame == 0) {
			throw_first_play_frame = 1;
			lowpassinit(out->config.rate);
 		}
		lowpass((void *)buf,out_frames,out->config.channels);
	//	ALOGD("PCM LOWPASS==  %d\n=====",out->config.channels );
	}
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    if (ret >= 0) {
	int val = atoi(value);
        if ((adev->mode == AUDIO_MODE_FM) && (adev->out_device  != val) && (val != 0)) {
		adev->out_device = val;
            	select_output_device(adev);
        }
    }
                     ...
                     hw_device_t** device)
{
   ...
   if (ril_dev_init() < 0 ){
    ...
    g_no_output = true;
#ifdef ANALOG_PHONE
    char plan_name[PROPERTY_VALUE_MAX]={0};
    ret = property_get("ro.sw.audio.codec_plan_name", plan_name, "0");
    if(!strncmp(plan_name, "PLAN_ONE", 8)){
		g_no_output = false;
    }
    ALOGD("g_no_output =%d", g_no_output);
#endif
